...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        const Online = message.guild.members.cache.filter(Mem => Mem.presence.status === "online"),
            Offline = await message.guild.members.cache.filter(Mem => Mem.presence.status === "offline"),
            Idle = await message.guild.members.cache.filter(Mem => Mem.presence.status === "idle"),
            Dnd = await message.guild.members.cache.filter(Mem => Mem.presence.status === "dnd");
        ...
        let SameTag = await message.guild.members.cache.map(Mem => Mem.user.discriminator),
            Fake = [],
            Original = [];
